...
class KDDragAndDropCollectionView: UICollectionView, KDDraggable, KDDroppable {
    ...
    func getIndexdrop ( _ rect : CGRect) {
        ...
        if let cellRetrieved = cellCandidate {
            print(self.indexPath(for: cellRetrieved)!.row)
          
            let realm = try! Realm()
            
            let profilliste = realm.objects(ProfilsListe.self)
           // finir ce soir
            print(profilliste[self.indexPath(for: cellRetrieved)!.row].name)
            
            
            
            UserManager.sharedInstance.setMoodsSelects(_moodsselects: profilliste[self.indexPath(for: cellRetrieved)!.row].name)
    ...
    func indexPathForCellOverlappingRect( _ rect : CGRect) -> IndexPath? {
        ...
        if visibleCells.count == 0 {
            //print(IndexPath(row: 0, section: 0) )
            return IndexPath(row: 0, section: 0)
        }
        
        if  isHorizontal && rect.origin.x > self.contentSize.width ||
            !isHorizontal && rect.origin.y > self.contentSize.height {
           // print(IndexPath(row: visibleCells.count - 1, section: 0))
            //print(ProfilslisteManager.sharedInstance.getProfilsListes())
