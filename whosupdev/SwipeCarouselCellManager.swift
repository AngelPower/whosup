...
struct SwipeCarouselCellManager {
    
    fileprivate var cell: SwipeCarouselCollectionViewCell!
    
    init(withCell: SwipeCarouselCollectionViewCell) {
        self.cell = withCell
    }
    ...
    mutating func handlePanGesture(_ sender: UIPanGestureRecognizer) {
        ...
        switch sender.state {
        case .began:
            self.cell?.delegate?.beginSwipe(self.cell!)
            originalPoint = cell.center //Get the center of the Cell.
        case .changed:
            
            self.cell?.delegate?.onSwiping(self.cell!, Yposition: swipeDistanceOnY)
    ...
    func afterSwipeAction() {
        ...
        NSLog("%f %f", swipeDistanceOnY, UIScreen.main.bounds.height/5)
        if swipeDistanceOnY > UIScreen.main.bounds.height/5 {
            downAction()
        }else{
            UIView.animate(withDuration: Constants.CenterImageAnimationDuration, animations: {
                self.cell?.center = self.originalPoint
    ...
    func downAction() {
        ...
        self.cell?.delegate?.cellSwipedDown(self.cell!)
        UIView.animate(withDuration: Constants.CenterImageAnimationDuration, animations: {
            self.cell?.center = self.originalPoint
        })
